...
public class DayOfWeekFunction implements FelFunction {
    @Override
    public Value apply(List<Value> values) {
        if (values.size() != 1) {
            throw new FilterException("dayOfWeek function expects exactly one argument");
        }

        var value = values.get(0).asDateTime();
        var dayOfWeek = value.getDayOfWeek().getValue();

        return new LongValue((long) dayOfWeek);
